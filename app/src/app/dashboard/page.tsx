...
export default function CreatorDashboard() {
    ...
    try {
      ...
      const response = await fetch('/api/content', {
        ...
        body: JSON.stringify({
          ...
          priceUSDC: (parseFloat(priceUSDC) * 1000000).toString(), // Convert to USDC decimals
        }),
      });
  ...
  const generateShareUrl = (contentId: string) => {
    ...
    return `${baseUrl}/api/app?creatorHandle=${encodeURIComponent(creatorHandle)}&contentId=${encodeURIComponent(contentId)}`;
  };
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            {contents.length === 0 ? (
              <p className="text-gray-500 text-center py-8">
                No content created yet. Create your first piece of premium content!
              </p>
            ) : (
              <div className="space-y-4">
                {contents.map((content) => (
                  <div key={content.contentId} className="border border-gray-200 rounded p-4">
                    <h3 className="font-medium text-gray-900">{content.title}</h3>
                    <p className="text-sm text-gray-600 mt-1">
                      Price: {(parseInt(content.priceUSDC) / 1000000).toFixed(2)} USDC
                    </p>
                    ...
                    {creatorHandle && (
                      <div className="mt-3">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          />
                          <button
                            onClick={() => copyToClipboard(generateShareUrl(content.contentId))}
